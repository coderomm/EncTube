...
function EditorRegister() {
    ...
    const handleRegister = async (e) => {
        ...
        if (username.length < 4) {
            toast.warning('Username must be at least 4 characters long.');
            return;
        }

        const passwordRegex = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{6,}$/;
        if (!passwordRegex.test(password)) {
            toast.warning('Password must be at least 6 characters long and include at least one letter and one number.');
            return;
        }
